...
contains
  ...
  subroutine mortality_rates( cohort_in,bc_in,cmort,hmort,bmort,frmort,smort,asmort,inmort)
 ...
 subroutine Mortality_Derivative( currentSite, currentCohort, bc_in, frac_site_primary)
    ...
    if (currentCohort%canopy_layer > 1)then 
       ...
       currentCohort%dndt = -1.0_r8 * &
            (cmort+hmort+bmort+frmort+smort+asmort + inmort + dndt_logging) &
            * currentCohort%n
    else
       ...
       currentCohort%dndt= -(1.0_r8-fates_mortality_disturbance_fraction) &
            * (cmort+hmort+bmort+frmort+smort+asmort+inmort) * &
            currentCohort%n
