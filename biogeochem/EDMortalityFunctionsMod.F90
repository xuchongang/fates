...
contains
  ...
  subroutine mortality_rates( cohort_in,bc_in,cmort,hmort,bmort,frmort,inmort )
    ...
    real(r8),intent(out) :: inmort ! insect induced mortality (rate per day)*(365 days) where the daily rate (0.0 <= inmort/365 <= 1.0)
 ...
 subroutine Mortality_Derivative( currentSite, currentCohort, bc_in)
    ...
    if (currentCohort%canopy_layer > 1)then 
       ...
       currentCohort%dndt = -1.0_r8 * (cmort+hmort+bmort+frmort+inmort+dndt_logging) * currentCohort%n
    else
       ...
       currentCohort%dndt = -(1.0_r8 - fates_mortality_disturbance_fraction) &
            * (cmort+hmort+bmort+frmort+inmort) * currentCohort%n
    endif
